...
### 🤝 D. 친선 경기 & 실시간 랭킹 시스템 (`js/friendly.js`)
* **타임아웃 경쟁**: Firestore 유저 로스터를 불러올 때 화면 멈춤을 방지하기 위해 `Promise.race` 5초 타임아웃 제한 적용.
* **오프라인 캐싱 폴백**: API 성공 시 로스터를 캐싱하고, 타임아웃/에러 시 캐시 데이터를 복원하여 중단 없는 매칭 지원.
