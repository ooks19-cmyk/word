...
5. **사용자 전용 엑셀 파일(`선수데이터1.xlsx`, `선수데이터2.xlsx`) 수정 금지**:
   - `선수데이터1.xlsx`, `선수데이터2.xlsx` 파일은 **사용자가 직접 편집 및 열람 시 사용하는 파일**입니다.
   - AI 에이전트는 해당 파일을 수정할 필요가 없으며, 임의로 수정하거나 덮어쓰지 않습니다.
6. **Git Push(깃 푸시) 원칙 (요청 시 진행)**:
   - Git Push(`git push`) 작업은 에이전트가 임의로 실행하지 않으며, **사용자의 명시적인 요청이 있을 때만 진행**합니다.
...
├── 선수데이터1.xlsx / 선수데이터2.xlsx                 # 👤 사용자 편집/참고용 엑셀 파일 (AI 수정 불필요)
...
* **출제 방식 (항상 4지선다형 객관식)**:
  * 요일 및 날짜와 무관하게 **모든 퀴즈 문제는 4지선다형 객관식**으로 출제됩니다.
* **요일별 단어 풀 분기**:
  * **📅 목요일·금요일 (복습 데이)**: 지금까지 학습한 **누적 과거 단어 풀에서 무작위 5문제를 선정**하여 출제 (`isReviewDay() === true`).
  * **📖 일반 요일 (토·일·월·화·수)**: 오늘 날짜($YYMMDD$) 기준 스케줄된 단어 세트에서 **무작위 5문제** 출제.
* **레벨업 및 보상 체계**:
  * 퀴즈 1회(5문제 완수) 시 `userLevel` 1 증가 및 FP 획득 (일반 +1 FP, 어려움 모드 +2 FP).
  * **Lv. 2**: 목표 달성 축하 팝업.
  * **레벨 10단위 특별 보상 (일괄 전설 카드 1장 / 올보유 시 10 FP)**:
    * 레벨이 10 상승할 때마다(`level % 10 === 0`) 미보유 **전설(Legend) 카드 중 무작위 1장** 확정 지급.
    * 이미 모든 전설 카드를 보유 중인 경우 **10 FP (가차 포인트)** 보너스 지급.
