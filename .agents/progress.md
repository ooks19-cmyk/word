## 현재 작업
- 목표: 국대 선수배치 팝업을 첨부 UI처럼 구성하고, 스타일 토글 시 드로어 재렌더링을 제거.
- 상태: 완료.
- 주요 변경·검증: 상단은 `선수 배치하기 (포지션)` 제목과 X 닫기 버튼으로 구성했다. 공격수 선택 시 한 줄 스타일 토글, 배치 해제, 카드 목록 순으로 표시하며, 불필요한 국적·실제 포지션 안내와 별도 선수 선택 제목은 제거했다. 토글은 저장값·상태 텍스트·토글만 즉시 갱신하고 드로어 전체를 다시 렌더링하지 않으며, 슬라이더 전환 애니메이션도 제거했다. 국대 포메이션 피치의 LW·ST·RW 카드 우상단에는 저장된 스타일을 나타내는 배지를 추가했다(드리블 돌파·치고 달리기·타겟맨·라인브레이커). 스타일은 국대 시즌 상태뿐 아니라 국가별 국대 프리셋에도 저장해 로그인 시 상태가 재생성되어도 복원된다. 60초 클라우드 저장 제한은 유지한다. 국대·클라우드 저장·정포지션·업적 회귀 테스트, JS 문법 및 Windows CRLF 허용 diff 검사를 통과했다. national_squad v2.5·national v3.8·global css v3.0·PWA v405.
- 다음 단계: 스타일 변경 후 60초 뒤 재로그인하여 프리셋 복원을 브라우저에서 육안 확인. 국대 스타일 변경은 `c66f48f`로 `origin/main`에 푸시됨. `선수데이터.csv`는 파일 잠금 해제 후 동기화.
