## 현재 작업
- 목표: 덱 화면에서 보관함 전환 시 현재 스크롤 위치 유지
- 상태: 완료
- 주요 변경·검증: 보관함 전환 전 스크롤 좌표·덱 높이를 저장하고, 렌더링 다음 프레임에 좌표를 복원하도록 수정. 덱 높이로 보관함 최소 높이를 유지해 스크롤 상단 이동 방지.
- 검증: 전환 함수 시뮬레이션에서 스크롤 좌표 480px·최소 높이 960px 보존, JS 문법·CRLF를 고려한 diff 검사 통과.
- 다음 단계: 완료. 801fc4e를 origin/main에 푸시.
