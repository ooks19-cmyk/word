## 현재 작업
- 목표: 국내 시즌 종료 뒤에만 국대 대회를 진행하고, 국대 대회 종료 후 다음날 새 시즌으로 전환.
- 상태: 완료. 국대모드 하위 네비게이션 버튼은 준비중 안내를 표시하며, 국대 종료 후 새 시즌 전환은 공용 버튼으로만 제공된다.
- 주요 변경·검증: 리그 종료 모달의 다음 시즌 버튼을 `국대 대회 진행하기`로 바꾸고 국대 탭을 연다. 국대모드 하위 네비게이션 버튼을 직접 누르면 `국대 모드는 현재 준비 중입니다.` 토스트를 표시한다. 시즌 종료 전에는 대회 시작 버튼을 누를 수 있으나 `시즌 종료후 시작 가능합니다` 토스트와 함께 차단된다. 국대 대회 종료 시에만 리그·컵·챔스·국대모드 하위 네비게이션 아래 공용 새 시즌 버튼을 표시한다. 종료 당일에는 버튼이 가능한 날짜를 표시하고 전환을 차단하며, 실제 다음 날짜부터 새 시즌 시작으로 바뀐다. 우승 축하 모달은 새 시즌 시작 없이 `확인` 버튼으로 닫는다. 로그인·새로고침 시 자동 시즌 종료 모달 복구를 제거하고, 이미 저장된 국대 대회가 있으면 국대 진입 처리도 상태를 변경하지 않는다. 국대·정포지션·업적 회귀, JS 문법 및 diff 검사 통과. league v4.4·national v3.7·style v2.0·PWA v397.
- 다음 단계: 새 서비스 워커 캐시 적용 후 브라우저에서 국대 종료 뒤 공용 버튼의 표시·다음날 전환·우승 모달 확인 닫기를 육안 확인. 커밋·푸시는 사용자 지시 대기.
- 다음 단계: 새 서비스 워커 캐시 적용 후 브라우저에서 진행 중·종료 후 국대 대회 계정의 로그아웃/로그인 뒤 대진·결과·다음 시즌 대기일이 유지되는지 육안 확인. 커밋·푸시는 사용자 지시 대기.
