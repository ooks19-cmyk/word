...
* **레벨 보상**: 퀴즈 1회(5문제 완수) 시 `userLevel` 1 증가.
  * **Lv. 2**: 목표 달성 안내 팝업 출력.
  * **Lv. 10 / 20 / 30 / 40 / 50 / 60 / 70**: 각각 이승우 / 손흥민(전설) / 이강인(전설) / 이승우(스페셜) / 박지성(전설) / 기성용(전설) / 김민재(전설) 지급 (보유 시 각성 +1).
  * **Lv. 80, 90... (10배수)**: DB 내 무작위 1명 지급 (보유 시 각성 +1).
...
### 🤝 D. 친선 경기 상대 목록 로드 & 캐싱 폴백 (`js/match.js`)
...
* **캐시 및 오프라인 폴백**: API 성공 시 로스터를 `fc_star_friendly_users_cache` 키로 캐싱. 타임아웃/에러 시 캐시를 복구하여 매칭을 중단 없이 지원하고, 캐시 로드 성공 시 네온 오렌지 Toast 경고를 노출함.
