...
function updateCupScoreboard() {
    ...
    if (analysisCard) {
        ...
        document.getElementById('cupOpponentFormationText').innerText = oppFormation;
        document.getElementById('cupOpponentMoodText').innerHTML = `보통 😐`; // 컵 대회 컨디션 보통 고정
        
        const compTextEl = document.getElementById('cupOpponentCompatibilityText');
        if (compTextEl) {
                ...
                if (specialEvent) {
                    ...
                    if (specialEvent.type === "pk_player") {
                        ...
                        if (isGoal) {
                            ...
                            addPlayerStatRecord(isHome ? playerMatch.team2 : playerMatch.team1, oppGoalData.scorerName, oppGoalData.assisterName);
            ...
            if (playerScoreVal === opponentScoreVal) {
                ...
                setTimeout(() => {
                    const etData = {
                        ...
                        score2: isHome ? opponentScoreVal : playerScoreVal,
        ...
        etResult.events.forEach(ev => {
            if (ev.type === 'goal') {
                ...
                if (isGoalByPlayer) {
                    ...
                    addPlayerStatRecord(isHome ? playerMatch.team2 : playerMatch.team1, ev.scorerName, ev.assisterName);
                }
            }
...
function addPlayerStatRecord(team, scorerName, assisterName) {
    ...
    if (assisterName) {
        ...
        // AI 팀에 어시스트 실명이 없고, 50% 확률에 격발될 때만 임의 에이스 적립
        if (Math.random() < 0.5) {
...
function finalizeCupMatch(score1, score2, playerMatch, pkScore1 = undefined, pkScore2 = undefined) {
    ...
    if (isPlayerWinner) {
        ...
        if (cupState.round === 8) {
            rewardPoints = 5;
            rewardText = "8강 진출 보상 5 FP를 획득했습니다!";
        } else if (cupState.round === 4) {
            rewardPoints = 5;
