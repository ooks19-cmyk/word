...
function startFriendlyMatchSimulation() {
    ...
    addCommentary('SYSTEM', `⚽ 🤝 친선 경기 매칭 전력 분석 | 나의 구단 OVR ${playerOvr} vs 상대 ${opponent.name} OVR ${oppOvr} (홈 ADV: 0)`, 'system');
    ...
    const finishFriendlyMatch = () => {
        ...
        addCommentary('FT', `삐- 삐- 삐--! 주심의 경기 종료 휘슬 소리와 함께 친선 경기가 종료됩니다!`, 'system');

        if (isWinner) {
            addCommentary('FT', `승리!!! 나의 구단이 완벽한 전술 제어로 상대 ${opponent.name}를 ${playerScoreVal} - ${opponentScoreVal}로 제압합니다! 🏆`, 'goal');
            friendlyMatchesHistory.w += 1;
            friendlyMatchesHistory.pts += 3;
        } else if (isDraw) {
            addCommentary('FT', `무승부! 접전 끝에 양 팀 ${playerScoreVal} - ${opponentScoreVal} 스코어로 승부를 가리지 못했습니다.`, 'system');
            friendlyMatchesHistory.d += 1;
            friendlyMatchesHistory.pts += 1;
        } else {
            addCommentary('FT', `패배! 상대 ${opponent.name}의 화력에 수비 벽이 뚫려 ${playerScoreVal} - ${opponentScoreVal} 아쉬운 패배를 맛봅니다.`, 'normal');
    ...
    if (isDeveloperMode) {
        matchMinutes.forEach(currentMin => {
            if (currentMin === 0) {
                addCommentary(0, `주심의 힘찬 휘슬 소리와 함께 친선 경기가 시작됩니다! 양 팀 당찬 표정으로 첫 그라운드를 밟습니다.`, 'normal');
            } else if (eventMins.includes(currentMin)) {
                const isPlayerAttack = Math.random() < playerAttackProb;
                if (isPlayerAttack) {
                    ...
                    addCommentary(currentMin, `상대팀 ${opponent.name}가 중원에서 패스워크를 맞추며 우리 진영을 위협합니다. 문전 앞 혼전 상황!`, 'attack');
                    if (isGoal) {
                        opponentScoreVal++;
                        addCommentary(currentMin, `실점! 상대 공격수의 기습적인 헤더 슛이 ${activeGk} 골키퍼의 손끝을 스치며 골문으로 밀려 들어갑니다.`, 'normal');
                    } else {
                        const gkSaveTexts = [
                            `${activeGk} 골키퍼의 빛나는 판단력! 침착하게 날아오는 크로스를 캐칭해 냅니다. 위기를 넘깁니다!`,
                            `미친 세이브!!! 수호신 ${activeGk} 골키퍼가 한 마리 새처럼 날아올라 손끝으로 공을 쳐냅니다! 멋진 슈퍼세이브! 🧤`
                        ];
                        const selectedGkSaveText = gkSaveTexts[Math.floor(Math.random() * gkSaveTexts.length)];
                        addCommentary(currentMin, selectedGkSaveText, 'normal');
                    }
                ...
                addCommentary('HT', `치열한 공방 끝 전반전이 종료되었습니다. 스코어 ${playerScoreVal} - ${opponentScoreVal}`, 'system');
            }
        });
    ...
    const matchTimer = setInterval(() => {
        ...
        if (currentMin === 0) {
            addCommentary(0, `주심의 킥오프 휘슬 소리와 함께 친선 경기가 개시됩니다!`, 'normal');
        } else if (eventMins.includes(currentMin)) {
            const isPlayerAttack = Math.random() < playerAttackProb;
            if (isPlayerAttack) {
                let attackOptions = [0, 1, 2];
            ...
            addCommentary('HT', `치열했던 전반전 종료. 현재 스코어 ${playerScoreVal} - ${opponentScoreVal} 양 팀 휴식에 들어갑니다.`, 'system');
        }
