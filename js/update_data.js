...
const UPDATE_LOGS = [
    ...
    },
    {
        ...
        items: [
            "<strong>친선모드 ➡️ 10단계 스테이지 '도전모드(Challenge Mode)' 전면 개편!</strong>: 매일 유럽/세계 최강의 명문 구단들과 격돌하는 10단계 스테이지 도전모드가 정식 출시되었습니다.<br>• <strong>10개 스테이지 명문 구단 라인업 (OVR 90~98)</strong>: 셀틱 FC(90), 아약스(91), SL 벤피카(92), AS 로마(93), 아틀레티코 마드리드(94), 아스날 FC(95), 바이에른 뮌헨(96), 맨체스터 시티(97), 파리 생제르맹(97), 레알 마드리드(98 FINAL BOSS)<br>• <strong>진행 및 5P 재도전 시스템</strong>: 하루 1경기 무료 도전 가능하며 승리 시 다음 스테이지로 진출. 실패 시 당일 1회에 한해 5 FP를 소모하여 즉시 재도전 가능<br>• <strong>리그/컵대회 정규 중계 엔진 & 무승부 승부차기(PK) 연동</strong>: 포지션별(LW/RW/ST/CM) 공격 전개, 실제 골키퍼 슈퍼세이브, 돌발 변수(PK/레드카드) 및 90분 무승부 시 승부차기 풀 루틴 완벽 구현<br>• <strong>시즌 1 우승 보상 '⚡ S메시 (★6각성)' 카드 지급</strong>: 10개 스테이지 전승 우승 시 기존 메시보다 오버롤 +1, 슈팅 +2 및 기본 6각성(실질 OVR 100)이 적용된 전용 특별 카드 즉시 덱 지급!<br>• <strong>슈퍼 카드 전용 블링블링 홀로그램 & 일반 뽑기 제외</strong>: 슈퍼(SUPER) 등급 카드에 오로라 쉬머와 스파클링 반짝임 애니메이션 탑재 및 일반 카드 팩 뽑기에서 제외하여 우승 보상만의 희소성 극대화<br>• <strong>데이터 동기화 사용자 선택 모달</strong>: 기기 간 데이터 불일치 감지 시 로컬 vs 클라우드 실시간 비교 카드를 제공하고 사용자가 직접 선택하도록 안전 조율 시스템 구축"
        ]
    },
    {
        version: "v2.8.1",
    ...
    },
    {
        ...
        items: [
            "<strong>프리미어리그 모드 'UEFA 챔피언스리그(UCL)' 정식 출시!</strong>: 프리미어리그(리버풀 FC) 모드에서 유럽 최고 권위의 16강 토너먼트 챔피언스리그가 오픈되었습니다.<br>• <strong>유럽 12대 명문 구단 및 스타 플레이어 총출동</strong>: 바이에른 뮌헨(<strong>김민재</strong>, 케인, 무시알라), 아틀레티코 마드리드(<strong>이강인</strong>, 그리즈만, 알바레스), 레알 마드리드(음바페, 비니시우스, 벨링엄), 바르셀로나(야말, 레반도프스키), PSG(뎀벨레, 바르콜라), 인터 밀란(라우타로), 레버쿠젠(비르츠), 도르트문트, 유벤투스, AC 밀란, 스포르팅 CP, 벤피카 참여<br>• <strong>EPL 4개 구단 분산 매칭 16강 브라켓</strong>: 리버풀 및 EPL 상위 3팀이 16강 4개 구역에 균등 분산되어 유럽 명문들과 격돌하는 박진감 넘치는 단판 토너먼트 시스템<br>• <strong>데이터 무결성 및 독립 클라우드 세이브</strong>: K리그(AFC 챔피언스리그)와 EPL(UEFA 챔피언스리그)의 토너먼트 진행도, 스탯, 명예의 전당 성적이 완벽히 격리 보존되며 리그 순위표 안전 가드 탑재<br>• <strong>빅이어 트로피 및 20 FP 우승 보상</strong>: 우승 시 특별 빅이어 우승 연출과 함께 20 FP 지급 및 명예의 전당 트로피 룸 연동 지원"
        ]
    },
    {
        version: "v2.7.1",
    ...
    },
    {
        ...
        badgeText: "HOTFIX",
        items: [
            "<strong>친선경기(Friendly Match) 활성 리그 팀명 및 엠블럼 동적 연동</strong>: 프리미어리그(리버풀) 등 다른 리그 진행 시에도 친선경기에서 '전북'으로 고정 표기되던 문제를 수정하여, 현재 부임 중인 활성 리그의 팀명(리버풀 FC / 전북 현대)과 공식 엠블럼, 전술 상성 해설 및 주간 순위표가 일관되게 반영되도록 개선했습니다.",
