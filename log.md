...
### 🛠️ 84) 260802 영단어 퀴즈 스케줄러 그룹 추가 (2026-08-02)
* **배경 및 목적**:
  - 교재 1단원(Lesson 1, 5~6페이지)에 해당하는 신규 단어 22개를 영단어 퀴즈 시스템의 8월 2일 예약 스케줄에 맞춰 등록하고 PWA 캐시를 최신화하기 위함입니다.
* **반영 사항**:
  - **신규 단어 추가 및 예약 등록 (`quiz_data.js`)**:
    - `QUIZ_WORDS_BY_DATE` 객체에 `"260802"` 키를 새로 생성하여 요정/마법사, 위치/동작 관련 신규 22개 단어를 등록했습니다.
    - `QUIZ_WORDS` 전역 통합 백업 풀 하단에도 동일 단어 22개를 추가 이식했습니다.
  - **가이드 문서 업데이트 (`quiz_vocabulary_schedule.md`)**:
    - 스케줄러 및 백업 풀 목록에 `260802` 추가 내용을 반영하고, 총 수록 단어 수를 **600단어**로 갱신했습니다.
  - **PWA 캐시 및 스크립트 버전 최신화 (`index.html`, `sw.js`)**:
    - 스크립트 캐시 무효화를 위해 `index.html` 의 `quiz_data.js` 로드 버전을 `v=1.7`로 올렸습니다.
    - 모바일 오프라인 캐시 갱신을 강제하기 위해 `sw.js` 의 PWA 캐시명을 `'fc-star-v252'`로 상향했습니다.
