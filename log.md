...
### ⚙️ 85) 신규 전설 등급 카드 추가 (나상호_L, 황인범_L) (2026-08-04)
* **배경 및 목적**:
  - 기존 국가대표 핵심 자원인 나상호, 황인범 선수의 기량 상향(기존 능력치 +2)을 반영한 전설 등급(Legend) 버전을 식별자 `_L`로 구분하여 신규 수록하기 위함입니다.
* **반영 사항**:
  ...
  - **PWA 캐시 및 스크립트 버전 최신화 (`index.html`, `sw.js`, `js/update_data.js`)**:
    - 변경 데이터가 적용되도록 `index.html` 내 `player_data.js` 로드 버전을 `v=1.41`로, `update_data.js` 로드 버전을 `v=2.52`로 올렸습니다.
    - 서비스 워커 PWA 캐시 버전을 `'fc-star-v254'`로 올렸습니다.
