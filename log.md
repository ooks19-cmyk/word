...
* **반영 사항**:
  - **신규 스페셜 선수 카드 추가 (`player_data.js`, `선수데이터.csv`)**:
    - **세메뇨 (`semenyo`)**: RW 포지션, OVR 85, 속도 89, 슈팅 84, 패스 78, 드리블 85, 수비 45, 피지컬 84 스탯 설계 및 본머스 고유 스페셜 테마 적용. 이미지 `player2/세메뇨.webp`.
    - **다나카 아오 (`ao_tanaka`)**: CM 포지션, OVR 84, 속도 78, 슈팅 79, 패스 83, 드리블 82, 수비 78, 피지컬 80 스탯 설계 및 리즈 유나이티드 스페셜 테마 적용. 이미지 `player2/다나카 아오.png`.
  - **PWA 캐시 및 스크립트 버전 최신화 (`index.html`, `sw.js`, `js/update_data.js`)**:
