# C:\Users\ooks1\OneDrive\바탕 화면\축구카드\scratch\get_ooks_343_exact.ps1
...
# Gegenpressing check: 2 or more of ST, LW, RW have PAC >= 90
$fastAttackers = 0
$attackers = @("ST", "LW", "RW")
foreach ($pos in $attackers) {
    if ($awakenedSquad[$pos].pac -ge 90) {
        $fastAttackers++
    }
}
$isDetailedActive = $fastAttackers -ge 2
$detailedTacticBonus = 0.0
if ($isDetailedActive) {
    $detailedTacticBonus = 0.05
}

# 3-4-3 suitability check (dribble)
$suitabilityBonus = [Math]::Max(0.0, ($avgDri - 70) * 0.01)

# formation OVR completion check
$hasKeyPlayer = $cmDri -ge 80
$hasTeamTactic = $avgPac -ge 70 # 3-4-3 uses PAC for team tactic
$formationBonus = 0
...
Write-Host "Average PAC (Team Tactic): $avgPac"
Write-Host "Average DRI (Suitability): $avgDri"
Write-Host "Average DEF (Defense Avg): $avgDef"
Write-Host "CM DRI: $cmDri (Key Player: $hasKeyPlayer)"
Write-Host "Fast Attackers count: $fastAttackers (Gegenpressing Active: $isDetailedActive)"
